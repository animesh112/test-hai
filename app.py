...
app.config['MAIL_PASSWORD'] = 'Bh233PratapVihar'
	...
	def wrap(*args, **kwargs):
		if 'logged_in' in session:
			if session['user_role']=="student":
				print("session mai user role student hai")
				return f(*args, **kwargs)
			else:
				flash('You dont have privilege to access this page!','danger')
				return render_template("404.html") 
...
def register():
	...
	if request.method == 'POST':
		...
		print("session otp : ",session['tempOTP'])
		# print(session)
		return redirect('/verifyEmail')
		# render_template('verifyEmail.html' , userkaname = session['tempName'] )
			...
			if img_result["verified"] == True and password == password_candidate:
				...
				if results2 > 0:
					...
					if user_type == "student":
						print("redirecting to student index")
